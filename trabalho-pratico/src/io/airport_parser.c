...
void processar_linha_valida(char **campos, int num_campos) {
    if (num_campos < 5) return; // precisa pelo menos de código, nome, cidade, país, latitude
    ...
    printf("Aeroporto válido: código '%s', nome '%s', cidade '%s', país '%s', latitude %.6f, longitude %.6f, icao '%s', type '%s'\n",
           codigo, nome, cidade, pais, latitude, longitude, icao, type);
}
